...
const shouldAdd = (user: string, item: ItemRelation, summary: CycleSummary): boolean => {
...
const parseWorkItemTypeForUser = (user: string, items: ItemsRelation, summary: CycleSummary, indentation: number = 0): string[][] => {
  let table: string[][] = [];
  if (items === undefined) {
    return table;
  }
...
const parseUser = (user: string, summary: CycleSummary): string | null => {
...
const parseWorkItemTypeForTeam = (items: ItemsRelation, summary: CycleSummary, indentation: number = 0): string[][] => {
  let table: string[][] = [];
  if (items === undefined) {
    return table;
  }
...
const parseTeamSummary = (summary: CycleSummary): string => {
  let overallTable: string[][] = []
...
const App = (): JSX.Element => {
  ...
  const [value, setValue] = useState("**No cycle specified; Waiting...**");
  ...
  useEffect(() => {
    let newCycle = searchParams.get('cycle')
    if (cycle !== newCycle) {
      setCycle(newCycle ?? undefined);
  ...
  useEffect(() => {
    if (!team || team === null || !cycle || cycle === null || generateRequestSent) {
      return;
    }
    ...
    const action: GenerateCycleSummaryAction = {
      action: 'GenerateCycleSummary',
      cycle: cycle,
      team: team
    }
    chrome.runtime.sendMessage(action, (resp) => { });
  }, [team, cycle, generateRequestSent]);
