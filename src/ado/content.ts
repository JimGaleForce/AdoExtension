...
`;
    ...
    let cellClass = isNewAdoHub ? 'bolt-list-cell' : 'grid-cell';
    const pageContent = document.getElementsByClassName('page-content')[0];
    var rows = isNewAdoHub ? pageContent.getElementsByClassName('bolt-list-row') : document.getElementsByClassName('grid-row');
    if (rows.length === 0) {
        rows = isNewAdoHub ? pageContent.getElementsByClassName('taskboard-card') : document.getElementsByClassName('tbTileContent');
        if (rows.length > 0) {
